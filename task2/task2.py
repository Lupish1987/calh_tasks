...
class Target:
    ...
    def request(self, json_request):
        ...
        if 'customer_request' not in json_dict                 or 'customer' not in json_dict['customer_request']                 or 'cunbr' not in json_dict['customer_request']['customer']:
...
class XMLWebServiceInvoker:
    ...
    def request(self, cunbr):
        ...
        # IMPLEMENT ME PLZ!
        response = requests.get("https://coding-academy.pl/customer/"+str(cunbr))
...
class Adapter(Target):
    ...
    def request(self, json_request):
        ...
        tree = xml_web_service.request(cunbr)
        
        myTree= json.loads(tree)
        accounts = myTree['customer_data']['accounts']['account']
        
        json_respone = JsonResponseBuilder().with_cunbr_accounts(cunbr, accounts)
        return(json_respone)
...
class JsonResponseBuilder(AbstractJsonBuilder):
    ...
    def with_cunbr_accounts(self, cunbr, accounts):
        # IMPLEMENT ME PLZ!
        
        if isinstance(accounts, list):
            thisdict = {"customer_request": {"customer": {"cunbr": cunbr, "accounts": accounts}}}
        else:
            thisdict = {"customer_request": {"customer": {"cunbr": cunbr, "accounts": [accounts]}}}
...
class JsonRequestBuilder(AbstractJsonBuilder):
    ...
    def with_cunbr(self, cunbr):
        # IMPLEMENT ME PLZ!
        thisdict = {"customer_request": {"customer": {"cunbr": cunbr}}}
        return json.dumps(thisdict) 
